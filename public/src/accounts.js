...
function sortAccountsByLastName(accounts) {
  ...
  const sortedAccount = accounts.sort((accountA, accountB) => accountA.name.last.toLowerCase() > accountB.name.last.toLowerCase() ? 1 : -1);
...
function getAccountFullNames(accounts) {
  ...
  const accountFullName = accounts.map(({name:{first, last}}) => first + ' ' + last);
